...
|----------|------|---------|
| `Success` | `bool` | `true` for `Ok` and `Next`; `false` for `Fault` |
| `RouteNext` | `bool` | `true` if the router should continue describing handlers |
| `NextType` | `Type?` | When set, restricts continued routing to handlers of this exact type |
